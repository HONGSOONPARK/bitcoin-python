...
def start_selltrade(sell_pcnt, dcnt_pcnt):
    try:
        ...
        while True:
            ...
            # 보유 종목조회
            # ------------------------------------------------------------------
            target_items = commons.get_accounts("Y", "KRW")

            # ------------------------------------------------------------------
            # 보유 종목 현재가 조회
            # ------------------------------------------------------------------
            target_items_comma = commons.chg_account_to_comma(target_items)
            tickers = commons.get_ticker(target_items_comma)
                ...
                for ticker in tickers:
                    if target_item["market"] == ticker["market"]:
                        ...
                        rev_pcnt = round(
                            (
                                (
                                    Decimal(str(ticker["trade_price"]))
                        ...
                        # -------------------------------------------------
                        order_done = commons.get_order_status(
                            target_item["market"], "done"
                        ) + commons.get_order_status(target_item["market"], "cancel")
                        order_done_sorted = commons.orderby_dict(
                            order_done, "created_at", True
                        )
                        order_done_filtered = commons.filter_dict(
                            order_done_sorted, "side", "bid"
                        )

                        # ------------------------------------------------------------------
                        # 캔들 조회
                        # ------------------------------------------------------------------
                        candles = commons.get_candle(target_item["market"], "D", 200)
                        ...
                        mask = (
                            ...
                            > order_done_filtered[0]["created_at"]
                        )
                        ...
                        if Decimal(str(cur_dcnt_pcnt)) < Decimal(str(dcnt_pcnt)):
                            ...
                            # ------------------------------------------------------------------
                            logging.info(
                                "시장가 매도 시작! [" + str(target_item["market"]) + "]"
                            )
                            # rtn_sellcoin_mp = commons.sellcoin_mp(target_item['market'], 'Y')
                            logging.info(
                                "시장가 매도 종료! [" + str(target_item["market"]) + "]"
                            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        commons.set_loglevel(log_level)
